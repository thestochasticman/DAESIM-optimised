...
class LeafGasExchangeModule:
    ...
    Vqmax_opt_rVcmax: float = field(default=None)  ## Vqmax_opt as a ratio of Vcmax_opt, if not None, then the Vqmax_opt parameter is updated using this ratio
    Vqmax_opt_rVcmax_method: str = field(default="linear")  ## Method for relating Vqmax_opt to Vcmax_opt, ("linear", "log") (analagous to Jmax relationship as in Walker et al., 2014, doi: 10.1002/ece3.1173)
    ...
    def calculate(
        ...
        # Ensure Vqmax_opt is updated before running calculations, as it can be assigned direction using Vqmax_opt or set using a relationship with Vcmax_opt
        self.update_Vqmax_opt()
    ...
    def update_Vqmax_opt(self):
        """
        Updates Vqmax_opt based on the value of Vqmax_opt_rVcmax and Vcmax_opt.
        If Vqmax_opt_rVcmax is None, Vqmax_opt remains unchanged.

        Notes
        -----
        For Vqmax_opt_rVcmax_method == "log", there is a log-transformed relationship 
        between Vqmax_opt and Vcmax_opt as described in Walker et al. (2014). The 
        global relationship found in that study (as implemented in Walker et al., 
        2017) suggests that Vqmax = np.e * Vcmax**b, where b=0.89 which is the slope 
        parameter in the log-log transformed relationship. 

        N.B. we assume that Vqmax_opt is proportional to Jmax_opt and thus has a 
        similar correlation with Vcmax_opt as shown by observations. 
        ...
        """
        if self.Vqmax_opt_rVcmax is not None:
            if self.Vqmax_opt_rVcmax_method == "linear":
                # Linear relationship
                self.Vqmax_opt = self.Vqmax_opt_rVcmax * self.Vcmax_opt
            elif self.Vqmax_opt_rVcmax_method == "log":
                # Power law relationship (Walker et al., 2014; 2017)
                self.Vqmax_opt = np.e * self.Vcmax_opt**self.Vqmax_opt_rVcmax